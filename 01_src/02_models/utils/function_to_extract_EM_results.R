...
calculate_subgroup_effects <- function(model, exposure, modifier) {
  ...
  # all_levels <- unique(model$data[[modifier]])
  ...
  # levels_to_process <- unique(c(reference_level, modifier_levels))
  ...
  # Calculate RERI for each non-reference level
  ...
  for (i in 1:length(modifier_levels)) {
    ...
    # Calculate RERI standard error using delta method
    if (interaction_term %in% names(coefs) && modifier_term %in% names(coefs)) {
      beta_interaction <- coefs[interaction_term]
      se_interaction <- sqrt(vcov_matrix[interaction_term, interaction_term])
      se_modifier <- sqrt(vcov_matrix[modifier_term, modifier_term])
      
      # Get RR11 (joint effect)
      ...
      dRERIdbeta_exp <- RR11 - RR10
      dRERIdbeta_mod <- RR11 - RR01
      dRERIdbeta_int <- RR11
      
      # Covariances
      cov_exp_mod <- vcov_matrix[exposure, modifier_term]
      cov_exp_int <- vcov_matrix[exposure, interaction_term]
      cov_mod_int <- vcov_matrix[modifier_term, interaction_term]
      
      # Calculate SE
      RERI_SE <- sqrt(
        (dRERIdbeta_exp^2 * se_exposure^2) +
        (dRERIdbeta_mod^2 * se_modifier^2) +
        (dRERIdbeta_int^2 * se_interaction^2) +
        (2 * dRERIdbeta_exp * dRERIdbeta_mod * cov_exp_mod) +
        (2 * dRERIdbeta_exp * dRERIdbeta_int * cov_exp_int) +
        (2 * dRERIdbeta_mod * dRERIdbeta_int * cov_mod_int)
      )
    } else if (interaction_term %in% names(coefs)) {
      # If modifier main effect not in model
      beta_interaction <- coefs[interaction_term]
      se_interaction <- sqrt(vcov_matrix[interaction_term, interaction_term])
...
#' 
#' This function depends on the `calculate_subgroup_effects` function (not shown here)
#' which must extract odds ratios, interaction p-values, and RERI from the models.
#'
#' @note
...
process_models <- function(model_list, path_output) {
  ...
  for (modifier in unique_modifiers) {
    ...
    if (nrow(odds_results) > 0) {
      writeDataTable(wb_odds_ratios, sheet = sheet_name, 
                    x = odds_results,
    ...
    if (nrow(int_p_results) > 0) {
      writeDataTable(wb_interaction_p, sheet = sheet_name, 
                    x = int_p_results,
                    startRow = 1, startCol = 1,
                    tableStyle = "TableStyleMedium2")
    }
    ...
    if (nrow(reri_results) > 0) {
      writeDataTable(wb_reri, sheet = sheet_name, 
                    x = reri_results,
                    startRow = 1, startCol = 1,
                    tableStyle = "TableStyleMedium2")
    }
