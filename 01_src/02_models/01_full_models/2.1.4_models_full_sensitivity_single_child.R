...
source(here("paths.R"))
## Get output paths
output_rds <- here(path_project, "outputs", "models", "full_models", "sensi_single_child")
dir.create(output_rds, showWarnings = FALSE)
output_xlsx <- here(path_project, "outputs", "models", "full_models", "sensi_single_child")
dir.create(output_xlsx, showWarnings = FALSE)
...
nrow(df_paper_final)
length(unique(df_paper_final$caseid))

# Randomly select one child per household ----
set.seed(12345)
...
cat("Saving all models to:", basename(here(output_rds, "models_all_sensi.rds")), "\n")
saveRDS(all_models, here(output_rds, "models_all_sensi.rds"))

# Extract coefficients ----
cat("\nExtracting coefficients and saving to Excel...\n")
tic("Coefficient extraction")
process_models_to_excel(here(output_rds, "models_all_sensi.rds"), here(output_xlsx, "coefs_all_models_sensi.xlsx"))
toc()

cat("\nComplete! All results saved to:", basename(here(output_xlsx, "coefs_all_models_sensi.xlsx")), "\n")
