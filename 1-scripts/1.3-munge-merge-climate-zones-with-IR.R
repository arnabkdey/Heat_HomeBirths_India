...
rm(list = ls())
file_path_IR <- here("2-data", "2.2-processed-data", "1.1-dhs-IR-vars-created.fst")
df_IR_long <- read_fst(file_path_IR, as.data.table = TRUE)

## Climate zones of India with districts dataset
file_path_zones <- here("2-data", "2.2-processed-data", "1.2-india-dist-climate-zones.fst")
...
## Save the file
write_fst(df_IR_long_w_zones, path = here(path_processed_data, "1.3-dhs-IR-vars-climate-zones.fst"))
