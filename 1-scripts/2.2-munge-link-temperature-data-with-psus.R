...
df_dhs_psu_geo_sf <- readRDS(here(path_processed, "2.1-a-df-dhs-psu-geo.rds"))
india_boundary_buf <- readRDS(here(path_processed, "2.1-b-ind-boundary-0-buf.rds"))
# head(df_dhs_psu_geo_sf)

# Step-2: load-function to extract climate data to DHS PSUs ----
source(here("src", "5.4-function-to-extract-climate-data-for-psus.R"))
...
write_fst(df_psu_tmax_wb, path = here(path_processed, "2.2.1-df_psu_tmax_wb.fst"))
rm(df_psu_tmax_wb)
...
write_fst(df_psu_precip, path = here(path_processed, "2.2.2-df_psu_precip.fst"))
rm(df_psu_precip)
...
rm(list = ls())
## Read Tmax - WB
df_psu_tmax_wb <- read_fst(here(path_processed, "2.2.1-df_psu_tmax_wb.fst"), columns = c("psu", "date", "dist_name", "lat", "long", "max_temp_wb"))
min(df_psu_tmax_wb$date)
max(df_psu_tmax_wb$date)
## Read Precipitation
df_psu_precip <- read_fst(here(path_processed, "2.2.2-df_psu_precip.fst"), columns = c("psu", "date", "mean_precip"))
...
write_fst(df_psu_temp_precip, path = here(path_processed, "2.2-daily-temp-precip-1980-21-extracted-dhs-psu.fst"))
