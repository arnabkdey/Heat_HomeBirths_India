...
path_out <- here("3-outputs", "supplements", "full-models", "conf-level-97")
...
for(exposure in names(model_outputs)) {
  ...
  tidy_outputs[[exposure]] <- broom.mixed::tidy(model, exponentiate = TRUE, conf.int = TRUE, conf.level = 0.97)
...
saveWorkbook(wb, here(path_out, "models_full_97.xlsx"), overwrite = TRUE)
...
write.csv(combined_exposures, here(path_out, "models_consolidated_coefficients_97.csv"), row.names = FALSE)
...
write.csv(df_icc_r2, here(path_out, "model_metrics_97.csv"), row.names = FALSE)